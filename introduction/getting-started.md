...
> 💡 아래 3개는 **관리자 권한 명령 프롬프트(cmd)**에서 winget 명령 하나씩으로 설치합니다. 명령 프롬프트를 처음 열어보신다면 아래 순서를 따라 하세요.
>
> 1. 키보드에서 **윈도우 키**를 누릅니다 (시작 메뉴가 열립니다)
> 2. `cmd`라고 입력합니다 ("명령 프롬프트" 앱이 검색 결과에 나타납니다)
> 3. 검색 결과의 **"명령 프롬프트"** 위에서 **마우스 오른쪽 버튼**을 클릭합니다
> 4. 메뉴에서 **"관리자 권한으로 실행"**을 클릭합니다
> 5. "이 앱이 디바이스를 변경할 수 있도록 허용하시겠습니까?" 창이 뜨면 **"예"**를 클릭합니다
> 6. 검은 화면의 창이 열리면 준비 완료입니다 — 아래 명령들을 여기에 입력하세요
>
> 📸 _스크린샷 삽입: cmd를 관리자 권한으로 여는 과정_
