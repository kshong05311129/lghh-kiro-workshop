...
30분간 만든 결과물을 다 같이 공유하는 시간입니다. 인원 수에 맞게 진행자가 아래 중 편한 방식으로 진행해주세요.

* **인원이 적을 때(10명 내외)**: 한 명씩 화면 공유로 1~2분씩 발표
* **인원이 많을 때**: 자원자 몇 명만 발표 + 나머지는 노트북을 켜둔 채 자유롭게 돌아다니며 서로 결과물 구경(장터식)
